...
Artist.search = ({ id = "", name = "" }, result) => {
    const queryStr = `SELECT * FROM ${ARTIST_TABLE} WHERE ${COL_ID} = ${mysql.escape(id)} OR ${COL_NOMBRE} = ${mysql.escape(name)}`;
...
Artist.delete = (id, result) => {
    dbConn.query(`DELETE FROM ${ARTIST_TABLE} WHERE ${COL_ID} = ?`, [mysql.escape(id)], (error, response) => {
        if (error) return result({ error: error }, null);
        return result(null, { message: "Se ha eliminado correctamente al artista" });
    });
}
