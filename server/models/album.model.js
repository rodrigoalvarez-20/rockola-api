...
Album.getAll = (decade, order = "asc", result) => {
  dbConn.query(
    `SELECT * FROM ${ALBUM_TABLE} WHERE fecha > ${Number(decade)} AND fecha < ${
      Number(decade) + 10
...
Album.update = (
  { id, name, fecha, noCanc, portada, cportada, idEst, idArt },
  result
) => {
  dbConn.query(
    `UPDATE ${ALBUM_TABLE} SET ${COL_NOMBRE} = ?, ${COL_FECHA} = ?, ${COL_NUMCANC} = ?, ${COL_PORTADA} = ?, ${COL_CPORTADA} = ?, ${COL_IDESTUDIO} = ?, ${COL_IDARTISTA} = ? WHERE ${COL_ID} = ?`,
    [
      mysql.escape(name),
      mysql.escape(fecha),
      mysql.escape(noCanc),
    ...
    (error, res) => {
      if (error) return result({ error: error }, null);
      return result(null, {
        message: "Se ha actualizado correctamente el album",
      });
    }
  );
};
...
Album.delete = (id, result) => {
  dbConn.query(
    `DELETE FROM ${ALBUM_TABLE} WHERE ${COL_ID} = ?`,
    [mysql.escape(id)],
    (error, response) => {
      if (error) return result({ error: error }, null);
      return result(null, {
        message: "Se ha eliminado correctamente al album",
      });
    }
  );
};
