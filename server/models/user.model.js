...
User.register = (newUser, result) => {
  User.findByEmail(newUser.email, (error, usrFind) => {
    ...
    else if (Object.keys(usrFind).length !== 0)
      result(
        {
...
User.login = (user, result) => {
  User.findByEmail(user.email, (error, userFind) => {
    if (error) return result({ error: error }, null);
      ...
      return result(
        {
          error: "No se ha encontrado ningun usuario con el email",
          code: StatusCodes.NOT_FOUND,
        },
        null
      );
    bcrypt
      .compare(user.password, userFind.password)
      .then((correct) => {
        if (!correct)
          return result(
            {
...
User.findByEmail = (email, result) => {
  dbConn.query(
    `SELECT * FROM ${USERS_TABLE} WHERE ${COL_EMAIL} = ?`,
    email,
    (err, res) => {
