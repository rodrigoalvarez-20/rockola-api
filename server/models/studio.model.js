...
Studio.search = ({ id = "", name = "" }, result) => {
  const queryStr = `SELECT * FROM ${STUDIO_TABLE} WHERE ${COL_ID} = ${mysql.escape(
    id
...
Studio.update = ({ id, name, address, webpage, phone }, result) => {
  dbConn.query(
    `UPDATE ${STUDIO_TABLE} SET ${COL_NOMBRE} = ?, ${COL_DIRECCION} = ?, ${COL_SITIOWEB} = ?, ${COL_TELEFONO} = ? WHERE ${COL_ID} = ?`,
    [
      mysql.escape(name),
      mysql.escape(address),
      mysql.escape(webpage),
      mysql.escape(phone),
      mysql.escape(id),
    ],
    (error, res) => {
      if (error) return result({ error: error }, null);
      return result(null, {
        message: "Se ha actualizado correctamente el estudio de grabacion",
      });
    }
  );
};
...
Studio.delete = (id, result) => {
  dbConn.query(
    `DELETE FROM ${STUDIO_TABLE} WHERE ${COL_ID} = ?`,
    [mysql.escape(id)],
    (error, response) => {
      if (error) return result({ error: error }, null);
      return result(null, {
        message: "Se ha eliminado correctamente el estudio de grabacion",
      });
    }
  );
};
